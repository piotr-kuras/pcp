...
def parse(soup):
    ...
    for item in records:
        product = {
            ...
            'price': item.find('span', {"class": "price-format nowrap"}).text.replace("zł", "").replace(",", ".").replace(" ", ""),
            "url": "https://www.ceneo.pl"+item.find('a', {"class": "button button--primary button--flex go-to-shop"})['href'],
        }
...
def loadup(products_list, product):
    for item in products_list:
        print(product)
        shop = Shop.objects.get_or_create(name=item['shop'], product=product, product_url=item['url'])
        Price.objects.create(shop=shop[0], price=float(item['price'])).save()
